...
async function processExactFieldMappings() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (rowFields.length > 0) {
      ...
      let order = existingLinks.length + 1
      
      for (const fieldLabel of rowFields) {
        if (!existingFieldNames.has(fieldLabel)) {
          ...
          console.log(`  - Field already linked via template: ${fieldLabel}`)
        }
      }
    }
  }
...
function guessFieldType(fieldLabel: string): string {
