...
async function main() {
  ...
  const defaultSupportGroup = await prisma.supportGroup.findFirst({
  ...
  if (existingBranches === 0) {
    ...
    console.log('Branches already exist, fetching...');
  ...
  for (const [key, name] of tier1Categories) {
    ...
    if (existing) {
      createdTier1.set(name, existing);
    } else {
  ...
  for (const [key, name] of tier1Categories) {
    ...
    if (existing) {
      legacyCategories.set(name, existing);
    } else {
  ...
  console.log(`Creating services from ${csvData.length} records...`);
  const services = [];
  let created = 0;
  let skipped = 0;
  ...
  // Create sample tickets only if we don't have any
  const existingTickets = await prisma.ticket.count();
  
  if (existingTickets === 0 && services.length > 0) {
  ...
  console.log(`- ${createdTier1.size} Tier 1 categories`);
  console.log(`- ${createdTier2.size} Tier 2 subcategories`);
  console.log(`- ${createdTier3.size} Tier 3 items`);
  console.log(`- ${services.length} services in database`);
}
