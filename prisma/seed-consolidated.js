...
async function main() {
    ...
    try {
        ...
        const { tier1Categories, tier2Categories, tier3Categories, legacyCategories } = await seed3TierCategories();
        ...
        const services = await seedServices(tier1Categories, tier2Categories, tier3Categories, legacyCategories, supportGroups[0] // Default support group
        );
...
async function seedBranchesAndATMs() {
    ...
    for (const branchData of branchesData) {
        const { atms: atmData } = branchData, branchInfo = __rest(branchData, ["atms"]);
...
async function seedUsers(branches, supportGroups) {
...
async function seedServices(tier1Categories, tier2Categories, tier3Categories, legacyCategories, defaultSupportGroup) {
    // Read CSV data
    const csvPath = path.join(process.cwd(), 'import1.csv');
    const csvData = parseCSV(csvPath);
    const services = [];
...
async function mapFieldTemplatesToServices(services, fieldTemplates) {
    var _a;
        ...
        if (matchingMappings.length > 0) {
            ...
            for (const mapping of matchingMappings) {
                for (const templateCategory of mapping.templates) {
                    const categoryFields = ((_a = commonFieldTemplates[templateCategory]) === null || _a === void 0 ? void 0 : _a.fields) || [];
...
async function seedSampleTickets(services, users, branches, defaultSupportGroup) {
        ...
        const sampleTicketsData = [
            {
                ...
                createdById: (_a = users.find(u => u.role === 'USER')) === null || _a === void 0 ? void 0 : _a.id,
                branchId: branches[0].id,
                supportGroupId: defaultSupportGroup.id,
            ...
            },
            {
                ...
                priority: 'HIGH',
                status: 'IN_PROGRESS',
                createdById: (_b = users.find(u => u.role === 'USER')) === null || _b === void 0 ? void 0 : _b.id,
                assignedToId: (_c = users.find(u => u.role === 'TECHNICIAN')) === null || _c === void 0 ? void 0 : _c.id,
                branchId: branches[0].id,
                supportGroupId: defaultSupportGroup.id,
            ...
            },
            {
                ...
                serviceId: ((_d = services.find(s => s.name.includes('Network'))) === null || _d === void 0 ? void 0 : _d.id) || services[0].id,
                priority: 'HIGH',
                status: 'OPEN',
                createdById: (_e = users.find(u => u.branchId === branches[1].id)) === null || _e === void 0 ? void 0 : _e.id,
                branchId: branches[1].id,
                supportGroupId: defaultSupportGroup.id,
                itilCategory: 'INCIDENT'
            },
            {
                ...
                serviceId: ((_f = services.find(s => s.name.includes('User'))) === null || _f === void 0 ? void 0 : _f.id) || services[0].id,
                priority: 'MEDIUM',
                status: 'RESOLVED',
                createdById: (_g = users.find(u => u.role === 'MANAGER')) === null || _g === void 0 ? void 0 : _g.id,
                assignedToId: (_h = users.find(u => u.role === 'TECHNICIAN')) === null || _h === void 0 ? void 0 : _h.id,
                branchId: branches[0].id,
                supportGroupId: defaultSupportGroup.id,
                itilCategory: 'SERVICE_REQUEST',
                resolvedAt: new Date()
        ...
        for (const ticketData of sampleTicketsData) {
            try {
                const ticket = await prisma.ticket.create({
                    data: Object.assign(Object.assign({}, ticketData), { createdAt: new Date(), updatedAt: new Date() })
                });
                tickets.push(ticket);
            }
            catch (error) {
