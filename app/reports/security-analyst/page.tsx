...
interface ReportData {
  ...
  filters: {
    ...
    securityAnalysts: Array<{ id: string; name: string; email: string }>
  }
}

export default function SecurityAnalystReport() {
  ...
  // Check access
  useEffect(() => {
    if (session && !['SECURITY_ANALYST', 'ADMIN', 'SUPER_ADMIN'].includes(session.user?.role)) {
      router.push('/reports')
    }
  }, [session, router])
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (createdById !== 'ALL') count++
    if (startDate) count++
    if (endDate) count++
    if (searchTerm) count++
    return count
  }, [status, priority, categoryId, branchId, createdById, startDate, endDate, searchTerm])
  ...
  }, [page, status, priority, categoryId, branchId, createdById, startDate, endDate, sortBy, sortOrder, session])
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/reports/security-analyst?${params}`)
      if (!response.ok) {
  ...
  const exportData = async () => {
    try {
      setExporting(true)
      const response = await fetch('/api/reports/security-analyst', {
        method: 'POST',
      ...
      a.download = `security-analyst-report-${format(new Date(), 'yyyy-MM-dd')}.${ext}`
      a.click()
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
              Security Analyst Tickets Report
            </h1>
          </div>
          <p className="text-sm text-muted-foreground ml-14">
            View and analyze all tickets created by or assigned to Security Analysts
          </p>
        </div>
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={createdById} onValueChange={setCreatedById}>
                      <SelectTrigger className="touch-target" aria-label="Filter by creator">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="ALL">All Security Analysts</SelectItem>
                        {data.filters.securityAnalysts.map((analyst) => (
        ...
        <Card>
          <CardContent className="p-12">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-1">No security analyst tickets found</h3>
