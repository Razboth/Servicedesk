...
// Validation schema for batch status updates using ticket numbers
const batchStatusUpdateSchema = z.object({
  ticketNumbers: z.array(z.string()).min(1).max(100),
  status: z.enum(['OPEN', 'PENDING', 'IN_PROGRESS', 'PENDING_VENDOR', 'RESOLVED', 'CLOSED', 'CANCELLED']),
  reason: z.string().optional(),
  resolutionNotes: z.string().optional()
});

// PUT /api/tickets/batch-status-by-number - Batch update ticket status using ticket numbers (session auth)
export async function PUT(request: NextRequest) {
  try {
    ...
    // Fetch all tickets by ticket numbers
    const tickets = await prisma.ticket.findMany({
      where: {
        ticketNumber: { in: validatedData.ticketNumbers }
      },
      select: {
        id: true,
        ticketNumber: true,
    ...
    // Track which ticket numbers were not found
    const foundTicketNumbers = tickets.map(t => t.ticketNumber);
    const notFoundTicketNumbers = validatedData.ticketNumbers.filter(
      tn => !foundTicketNumbers.includes(tn)
    );
    ...
    return NextResponse.json({
      ...
      requested: validatedData.ticketNumbers.length,
      found: tickets.length,
      notFound: notFoundTicketNumbers,
      skipped: tickets.length - ticketsToUpdate.length,
      message: `Successfully updated ${updateResult.count} ticket(s) to ${validatedData.status}`,
      details: {
        updatedTickets: ticketsToUpdate.map(t => t.ticketNumber),
        notFoundTickets: notFoundTicketNumbers,
        skippedTickets: tickets
          .filter(t => !ticketsToUpdate.some(tu => tu.id === t.id))
...
// PATCH /api/tickets/batch-status-by-number - Batch update with API key auth
