  ...
  try {
    ...
    for (const assignment of assignments) {
      try {
        ...
        // Check if THIS STAFF already has an assignment on this date (prevent duplicate staff per date)
        const existingStaffAssignment = await prisma.shiftAssignment.findFirst({
          where: {
            scheduleId,
            staffProfileId,
            date: assignmentDate,
        ...
        if (existingStaffAssignment) {
          // Same staff already assigned on this date - update the shift type
          ...
          // Create new assignment (allows multiple different staff on same shift type)
          ...
          // Auto-create Monday OFF after Sunday Night shift
          if (shiftType === 'SUNDAY_NIGHT') {
            const monday = new Date(assignmentDate);
            monday.setDate(monday.getDate() + 1); // Sunday + 1 = Monday

            // Check if Monday is still in the same month
            if (monday.getMonth() + 1 === schedule.month) {
              // Check if there's no existing assignment for this staff on Monday
              const existingMonday = await prisma.shiftAssignment.findFirst({
                where: {
                  scheduleId,
                  staffProfileId,
                  date: monday,
