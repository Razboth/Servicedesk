...
export class ShiftGenerator {
  ...
  async generateSchedule(
    ...
    // Step 4: Create LEAVE assignments for approved leave requests
  ...
  /**
   * Create LEAVE assignments for staff on approved leave
   */
  private createLeaveAssignments(): void {
    for (const [staffId, leaveDates] of this.leaveRequests.entries()) {
      for (const leaveDate of leaveDates) {
        // Only create LEAVE assignments for dates in this month
        if (leaveDate.getMonth() === this.month - 1 && leaveDate.getFullYear() === this.year) {
