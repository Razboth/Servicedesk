...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* User Profile Menu at Bottom - Cult UI Family Button Design */}
      <div className="border-t border-sidebar-border p-3 flex justify-center">
        {(!isCollapsed || isMobile) && (
          <FamilyButton>
            {/* Avatar as the button header (collapsed state) */}
            <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
              <Avatar className="h-12 w-12 ring-2 ring-amber-500/30 dark:ring-amber-400/30 shadow-lg">
            ...
            <div className="p-4 space-y-3">
              {/* Profile Header */}
              <div className="flex flex-col items-center gap-3 pb-3 border-b border-amber-200 dark:border-amber-800">
                <Avatar className="h-16 w-16 ring-4 ring-white/50 dark:ring-gray-800/50 shadow-xl">
                  {(session.user as any)?.avatar && getAvatarById((session.user as any).avatar) ? (
                    <div className="h-full w-full">
                      {getAvatarById((session.user as any).avatar)?.component}
                    </div>
                  ) : (
                    <AvatarFallback className="bg-gradient-to-br from-amber-500 via-orange-500 to-amber-600 text-white text-xl font-bold">
                      {session.user?.name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'U'}
                    </AvatarFallback>
                  )}
                </Avatar>
              ...
              {/* User Details */}
              <div className="space-y-2 text-xs pb-3 border-b border-amber-200 dark:border-amber-800">
                {session.user?.email && (
              ...
              <div className="space-y-1">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors text-sm"
                >
                ...
                <Link
                  href="/profile"
                  className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors text-sm"
                >
              ...
              <div className="pt-2 border-t border-amber-200 dark:border-amber-800">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors text-sm text-red-600 dark:text-red-400"
                >
        ...
        {/* Collapsed sidebar - show simple avatar button */}
        {isCollapsed && !isMobile && (
